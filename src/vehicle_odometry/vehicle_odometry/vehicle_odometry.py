...
class VehicleVisualOdometry(Node):
    def __init__(self):
        ...
        self.vehicle_global_position_sub = self.create_subscription(
            TagPosition, "/tag_position", self.__set_vehicle_odometry, 10
        )
        # Publisher
        self.publisher_ = [
            self.create_publisher(
                VehicleOdometry, f"/px4_1/fmu/in/vehicle_visual_odometry", 10
            ),
            self.create_publisher(
                VehicleOdometry, f"/px4_2/fmu/in/vehicle_visual_odometry", 10
            ),
            self.create_publisher(
                VehicleOdometry, f"/px4_3/fmu/in/vehicle_visual_odometry", 10
            ),
            self.create_publisher(
                VehicleOdometry, f"/px4_4/fmu/in/vehicle_visual_odometry", 10
            ),
        ]
    ...
    def publish_odometry(self, global_position, publisher_num):
        ...
        self.get_logger().info(
            f"px4_{publisher_num} Published visual odometry: {msg.position}"
        )
        self.timestamp = int(time.time() * 1e6)
