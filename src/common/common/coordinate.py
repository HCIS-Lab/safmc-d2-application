...
class Coordinate:
    ...
    def __str__(self) -> str:
        return f"Coord(x={self.x:.2f}, y={self.y:.2f}, z={self.z:.2f})"
    ...
    def clamp_magnitude(coord: "Coordinate", max_magnitude: float) -> "Coordinate":
        return (
            coord
            if coord.magnitude <= max_magnitude
            else max_magnitude * coord.normalized
        )
    ...
    def clamp_magnitude_2d(coord: "Coordinate", max_magnitude: float) -> "Coordinate":
        return (
            coord
            if coord.magnitude_2d <= max_magnitude
            else max_magnitude * coord.normalized_2d
        )

    @staticmethod
    def ned_to_enu(ned_coord: "Coordinate") -> "Coordinate":
        return Coordinate(x=ned_coord.y, y=ned_coord.x, z=-ned_coord.z)

    @staticmethod
    def enu_to_ned(enu_coord: "Coordinate") -> "Coordinate":
        return Coordinate(x=enu_coord.y, y=enu_coord.x, z=-enu_coord.z)
