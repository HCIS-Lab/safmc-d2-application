...
class bugNavigator:
    ...
    def compute_pos(self, current_position: Coordinate, target_position: Coordinate, obstacle_points: list[tuple[float, float]]):

        future_vel = self.move_in_direction_vel(current_position, target_position)
        valid = 1

        obstacle_points = list(filter(lambda obstacle: math.sqrt(
            ((obstacle[0] - future_vel.x)**2 + (-obstacle[1] - future_vel.y)**2)) < self.safe_dist, obstacle_points))
    ...
    def compute_tangent_pos(self, current_position: Coordinate, obstacle_points: list[tuple[float, float]]):
    ...
    def move_in_direction_vel(self, current_position: Coordinate, target_position: Coordinate):
