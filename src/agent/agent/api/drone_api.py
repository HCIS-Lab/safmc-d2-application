...
class DroneApi(Api):
    ...
    def publish_offboard_control_heartbeat(self, control_mode: str) -> None:
        ...
        offboard_control_mode_msg.acceleration = (
        ...
        offboard_control_mode_msg.thrust_and_torque = (
            control_mode == "thrust_and_torque")
        offboard_control_mode_msg.direct_actuator = (
            control_mode == "direct_actuator")

        if control_mode in ['position', 'velocity', 'acceleration', 'attitude', 'body_rate']:
            self.__node.get_logger().info(
                f"Control mode set to: {control_mode}")
        else:
