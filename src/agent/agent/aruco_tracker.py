    ...
    def image_callback(self, msg):
        ...
        if ids is not None:
            ...
            for rvec, tvec, marker_corner, id in zip(rvecs, tvecs, list(corners)[0], ids):
                ...
                marked_frame = frame
                cv2.line(marked_frame, (int(marker_corner[0][0]), int(marker_corner[0][1])), (int(
                    marker_corner[1][0]), int(marker_corner[1][1])), (0, 255, 0), 6)
                cv2.line(marked_frame, (int(marker_corner[1][0]), int(marker_corner[1][1])), (int(
                    marker_corner[2][0]), int(marker_corner[2][1])), (0, 255, 0), 6)
                cv2.line(marked_frame, (int(marker_corner[2][0]), int(marker_corner[2][1])), (int(
                    marker_corner[3][0]), int(marker_corner[3][1])), (0, 255, 0), 6)
                cv2.line(marked_frame, (int(marker_corner[3][0]), int(marker_corner[3][1])), (int(
                    marker_corner[0][0]), int(marker_corner[0][1])), (0, 255, 0), 6)
                ...
                cv2.putText(marked_frame, f"x:{tvec[0][1]:.3f}, y:{tvec[0][0]:.3f}", (int(marker_corner[3][0]), int(
                    marker_corner[3][1])-10), fontScale=1.5, thickness=3, fontFace=cv2.FONT_HERSHEY_SIMPLEX, color=(0, 255, 0), lineType=cv2.LINE_AA)
