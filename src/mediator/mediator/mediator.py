...
class Mediator(Node):
    ...
    def __set_is_armed_list(self, agent_info_msg):
        ...
        if not self.is_armed_list[index]:
    ...
    def __get_target_local_position(self, agent_local_position: NEDCoordinate, agent_global_position: NEDCoordinate, target_global_position: NEDCoordinate):
        if agent_local_position == None or agent_global_position == None or agent_global_position == None:
            return None
        # TODO 真的要 x, y 互換?
        # UWB / Gazebo 都是朝上, 但是 agent 是 NED (朝下)
        target_local_position = NEDCoordinate(
            agent_local_position.x + (target_global_position.y - agent_global_position.y),
            agent_local_position.y + (target_global_position.x - agent_global_position.x),
